 ...
 *
 * Version history:
 * 1.0.1 - Maintenance release: numeric attribute compatibility, chart metadata and logging polish.
...
@Field static final String TEMPERATURE_UNIT = "°C"
@Field static final String HUMIDITY_UNIT = "%"
@Field static final String ENERGY_UNIT = "kWh"
@Field static final String RUNTIME_UNIT = "min"
@Field static final List INTEGER_NUMERIC_ATTRIBUTES = ["humidity", "lastResponseCode"]
...
void updated() {
    ...
    unschedule()
    initializeLastManualFanMode()
    state.pendingNumericAttributeRepair = true
    repairExistingNumericStates()
    ensureChildDevices()
...
void refresh() {
    ...
    try {
        httpPost(params) { response ->
            ...
            sendNumberEventIfPresent("lastResponseCode", status, null, shouldForceNumericAttribute("lastResponseCode"))
            ...
            logTrace "Status response JSON: ${prettyJson(response.data)}"
            Map parsed = parseStatusResponse(response.data)
            applyParsedStatus(parsed)
        }
    } catch (Exception e) {
        markUnavailable(exceptionSummary(e))
    }
}
...
void applyParsedStatus(Map parsed) {
    ...
    if (parsed.powerOn != null) {
        sendEventIfChanged("switch", parsed.powerOn ? "on" : "off")
    }
    ...
    sendNumberEventIfPresent("temperature", parsed.currentTemperature, TEMPERATURE_UNIT, shouldForceNumericAttribute("temperature"))
    sendNumberEventIfPresent("humidity", parsed.currentHumidity, HUMIDITY_UNIT, shouldForceNumericAttribute("humidity"))
    sendNumberEventIfPresent("outsideTemperature", parsed.outsideTemperature, TEMPERATURE_UNIT, shouldForceNumericAttribute("outsideTemperature"))
    sendNumberEventIfPresent("thermostatSetpoint", parsed.thermostatSetpoint, TEMPERATURE_UNIT, shouldForceNumericAttribute("thermostatSetpoint"))
    sendNumberEventIfPresent("coolingSetpoint", parsed.coolingSetpoint, TEMPERATURE_UNIT, shouldForceNumericAttribute("coolingSetpoint"))
    sendNumberEventIfPresent("heatingSetpoint", parsed.heatingSetpoint, TEMPERATURE_UNIT, shouldForceNumericAttribute("heatingSetpoint"))
    sendNumberEventIfPresent("energyToday", parsed.energyToday, ENERGY_UNIT, shouldForceNumericAttribute("energyToday"))
    sendNumberEventIfPresent("runtimeToday", parsed.runtimeToday, RUNTIME_UNIT, shouldForceNumericAttribute("runtimeToday"))
    updateChildren(parsed)
    state.pendingNumericAttributeRepair = false
    state.repairedNumericAttributes = []
    ...
    if (logResponseSummary) {
        logInfo "Status: ${parsed.thermostatMode ?: "unknown"}, power ${formatPowerState(parsed.powerOn)}, " +
            "indoor ${formatMaybe(parsed.currentTemperature)} ${TEMPERATURE_UNIT}, target ${formatMaybe(parsed.thermostatSetpoint)} ${TEMPERATURE_UNIT}, " +
...
void sendNumberEventIfPresent(String name, Object value, String unit, Boolean forceEvent = false) {
    if (value == null) {
        return
    }
    Object numericValue = numericEventValueForAttribute(name, value)
    if (numericValue == null) {
        logWarn "Numeric attribute ${name} ignored because value is not numeric: ${value}"
        return
    }
    Map event = [name: name, value: numericValue]
    if (unit) {
        event.unit = unit
    }
    if (forceEvent) {
...
void repairExistingNumericStates() {
    List repairedAttributes = []
    numericAttributeUnits().each { String name, String unit ->
        Object currentValue = device.currentValue(name)
        if (currentValue != null) {
            Object numericValue = numericEventValueForAttribute(name, currentValue)
            if (numericValue == null) {
                logWarn "Existing numeric attribute ${name} could not be repaired because current value is not numeric: ${currentValue}"
            } else {
                Map event = [
                    name: name,
                    value: numericValue,
                ...
                sendEvent(event)
                repairedAttributes << name
                logDebug "Re-emitted numeric attribute ${name} during upgrade repair"
            }
        }
    }
    state.repairedNumericAttributes = repairedAttributes
}

Map numericAttributeUnits() {
    return [
        temperature: TEMPERATURE_UNIT,
        outsideTemperature: TEMPERATURE_UNIT,
        humidity: HUMIDITY_UNIT,
        thermostatSetpoint: TEMPERATURE_UNIT,
        coolingSetpoint: TEMPERATURE_UNIT,
        heatingSetpoint: TEMPERATURE_UNIT,
        energyToday: ENERGY_UNIT,
        runtimeToday: RUNTIME_UNIT,
        lastResponseCode: null
    ]
}

Boolean shouldForceNumericAttribute(String name) {
    List repairedAttributes = state.repairedNumericAttributes instanceof List ? state.repairedNumericAttributes : []
    return state.pendingNumericAttributeRepair == true &&
        numericAttributeUnits().containsKey(name) &&
        !repairedAttributes.contains(name)
}

Object numericEventValue(Object value) {
...
Object numericEventValueForAttribute(String name, Object value) {
    Object numericValue = numericEventValue(value)
    if (numericValue == null) {
        return null
    }
    if (INTEGER_NUMERIC_ATTRIBUTES.contains(name)) {
        return (numericValue as BigDecimal).setScale(0, BigDecimal.ROUND_HALF_UP) as Integer
    }
    return numericValue
}
...
Map childSensorDefinitions() {
    return [
        ...
        "energy": [
            label: "Energy Today",
            driver: "Daikin BRP084 Energy Child",
            attribute: "energyToday",
            standardAttribute: "energy",
            unit: ENERGY_UNIT,
...
void ensureChildDevices() {
    ...
    childSensorDefinitions().each { suffix, childDef ->
        ...
        if (childCreationEnabled(childDef) && !child) {
            createChildSensor(suffix, childDef)
        } else if (child) {
            validateExistingChildDriver(suffix, childDef, child)
        }
    }
}
...
void updateChildAttribute(String suffix, Object value) {
    ...
    Boolean childDriverOk = validateExistingChildDriver(suffix, childDef, child)

    Object numericValue = numericEventValue(value)
    if (numericValue == null) {
        logWarn "Child numeric attribute ${childDef.attribute} ignored because value is not numeric: ${value}"
        return
    }
    ...
    if (childDef.standardAttribute && childDriverOk) {
        Map standardEvent = [
            name: childDef.standardAttribute,
            value: numericValue
        ]
        if (childDef.unit) {
            standardEvent.unit = childDef.unit
        }
        child.sendEvent(standardEvent)
    }
}

Boolean validateExistingChildDriver(String suffix, Map childDef, Object child) {
    if (suffix != "energy" || child == null) {
        return true
    }

    String driverName = childDriverName(child)
    if (!driverName) {
        logDebug "Unable to confirm Energy Today child driver type"
        return true
    }
    if (driverName == childDef.driver) {
        state.energyChildDriverWarningLogged = false
        return true
    }

    if (state.energyChildDriverWarningLogged != true) {
        logWarn "Energy Today child is using ${driverName}, not ${childDef.driver}. Delete and recreate the Energy Today child to obtain the standard EnergyMeter capability and Home Page Charts support."
        logWarn "Upgrade steps: disable Create Energy Today Child, delete the existing Energy Today child, confirm Daikin BRP084 Energy Child is installed, re-enable Create Energy Today Child, save preferences or run Initialize, then Refresh."
        state.energyChildDriverWarningLogged = true
    }
    return false
}

String childDriverName(Object child) {
    try {
        return child.getTypeName() as String
    } catch (Exception ignored) {
    }
    try {
        return child.typeName as String
    } catch (Exception ignored) {
    }
    return null
}
...
Boolean sendWriteRequest(String target, List path, String parameter, String value) {
    ...
    try {
        ...
        httpPut(params) { response ->
            Integer status = response?.status as Integer
            sendNumberEventIfPresent("lastResponseCode", status, null)
            logDebug "Write HTTP status: ${status}"
            logTrace "Write response JSON: ${prettyJson(response.data)}"
            ...
            if (returnCode == 2004) {
                ...
                logWarn "Daikin BRP084 write rejected: unexpected DSIoT rsc=${returnCode}"
                success = false
            }
        }
        return success
    } catch (Exception e) {
        logWarn "Daikin BRP084 write failed: ${exceptionSummary(e)}"
        return false
    }
}
...
Map parseStatusResponse(Object responseData) {
    if (!(responseData instanceof Map)) {
        ...
        throw new IllegalArgumentException("Invalid JSON response: expected object, got null")
    }
    throw new IllegalArgumentException("Invalid JSON response: expected object")
    }
    Map data = responseData as Map
    if (!(data.responses instanceof List)) {
        throw new IllegalArgumentException("Unexpected DSIoT response: missing responses list")
    }
...
Object tryFindValueByPn(Map data, String fromPath, List names) {
    try {
        ...
        logDebug "DSIoT value not found at ${fromPath}: ${names.join(" > ")} (${e.message})"
        return null
    }
}
...
String exceptionSummary(Exception e) {
